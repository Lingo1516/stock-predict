...
def fetch_twse_stock_codes():
    ...
    # 將資料存儲為 pandas DataFrame
    df = pd.DataFrame(stock_data, columns=['股票名稱', '股票代號'])
    return df

# 將股票資訊存儲到內存（例如城市環境）以進行查詢
...
def get_stock_code(stock_name, stock_list):
    ...
    # 查找股票代號
    code_row = stock_list[stock_list['股票名稱'] == stock_name]
    if not code_row.empty:
        return f"{code_row.iloc[0]['股票代號']}.TW"
    else:
        # 提供詳細的錯誤提示，告知用戶無法找到該股票
...
def predict_next_5(stock, stock_list, days, decay_factor):
    try:
        ...
        if df is None or len(df) < 50:
            st.error(f"資料不足，僅有 {len(df) if df is not None else 0} 行資料")
            return None, None, None
        ...
        if len(df_clean) < 30:
            st.error(f"資料不足，僅有 {len(df_clean)} 行資料")
            return None, None, None
