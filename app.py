...
# ─────────────────────────────────────────────
# 蒙地卡羅模擬（修正：確保 sigma 夠大、up_prob 正確計算）
# ─────────────────────────────────────────────
def simulate_paths(df, n_paths, T, mean_revert, noise_mult) -> np.ndarray:
    close  = df["Close"].astype(float)
    ...
    # sigma：取 GARCH 與 rolling std 較大值，避免太小
    sigma_garch = get_sigma_forecast(df, T) * noise_mult
    sigma_roll  = float(df["SIGMA20"].iloc[-1])
    if not np.isfinite(sigma_roll) or sigma_roll <= 0:
        sigma_roll = max(float(ret.tail(60).std()), 0.005)
    sigma_t = np.maximum(sigma_garch, sigma_roll * noise_mult)

    # 動態偏移
    bias_unit = sigma_roll * 0.15
    ...
    try:
        ...
        df_t   = max(df_t, 2.5)
        scale_t = max(scale_t, sigma_roll * 0.5)  # 確保 scale 不為零
    except Exception:
    ...
    for t in range(T):
        raw_half = t_dist.rvs(df=df_t, loc=0,
                              scale=scale_t * max(sigma_t[t], sigma_roll * noise_mult),
                              size=half,
        ...
        paths[:, t] = prices.copy()   # ← 修正：必須 copy，否則 up_prob 計算錯誤
...
def make_short_forecast(last_close: float, fdates, paths: np.ndarray) -> list:
    results = []
    # 每日的比較基準：第0日與 last_close 比，第1日與第0日的路徑比
    for i in range(min(4, paths.shape[1])):
        day_paths = paths[:, i]                              # shape (n_paths,)
        base      = last_close if i == 0 else paths[:, i-1] # ← 修正關鍵

        med     = float(np.median(day_paths))
        p10     = float(np.percentile(day_paths, 10))
        p90     = float(np.percentile(day_paths, 90))
        up_prob = float(np.mean(day_paths > base) * 100)    # ← 逐路徑比較

        chg     = med - (last_close if i == 0 else float(np.median(paths[:, i-1])))
        chg_pct = chg / (last_close if i == 0 else float(np.median(paths[:, i-1]))) * 100

        if up_prob >= 58:
            direction, color = "⬆️ 偏漲", "#27AE60"
        elif up_prob <= 42:
        ...
        results.append({
            ...
            "up_prob": up_prob, "chg": chg, "chg_pct": chg_pct,
...
# ─────────────────────────────────────────────
# 圖一：清楚的卡片式漲跌機率長條圖（取代難懂的 Gauge）
# ─────────────────────────────────────────────
def build_prob_bar_chart(sf: list):
    labels   = [f"{r['label']}<br>{r['date']}" for r in sf]
    probs    = [r["up_prob"] for r in sf]
    colors   = [r["color"]   for r in sf]
    med_vals = [r["med"]     for r in sf]
    chg_pcts = [r["chg_pct"] for r in sf]
    dirs     = [r["direction"] for r in sf]
    ...
    fig.add_hline(y=50, line_dash="dash",
                  ...
                  annotation_text="50% 中性基準",
                  annotation_font_color="white")
    fig.add_hline(y=58, line_dash="dot",
                  line_color="rgba(39,174,96,0.6)",
                  annotation_text="58% 偏漲門檻",
                  annotation_font_color="#27AE60")
    fig.add_hline(y=42, line_dash="dot",
                  line_color="rgba(231,76,60,0.6)",
                  annotation_text="42% 偏跌門檻",
    ...
    for i, r in enumerate(sf):
        fig.add_annotation(
            x=labels[i], y=105,
            text=f"{r['direction']}<br><b>{r['med']:.2f}</b>（{r['chg_pct']:+.2f}%）",
            showarrow=False,
            font=dict(size=13, color=r["color"]),
            align="center",
        )
    ...
    fig.update_layout(
        ...
        title=dict(
            text="📊 次日 + 後三日｜上漲機率 & 預測股價",
            font=dict(size=16)
        ),
        yaxis=dict(title="上漲機率 (%)", range=[0, 115], ticksuffix="%"),
        xaxis=dict(title=""),
        showlegend=False,
...
def build_trend_chart(sf: list, last_close: float):
    ...
    fig.add_trace(go.Scatter(x=x_labels, y=y_p90, mode="lines",
        line=dict(color="rgba(100,180,255,0.4)", width=1, dash="dot"),
        name="90% 上限", showlegend=True))
    fig.add_trace(go.Scatter(x=x_labels, y=y_p10, mode="lines",
        line=dict(color="rgba(255,100,100,0.4)", width=1, dash="dot"),
        name="10% 下限", showlegend=True))
    ...
    for r in sf:
        fig.add_annotation(
            ...
            font=dict(color=r["color"], size=13),
        )

    fig.update_layout(
        height=380, template="plotly_dark",
        title=dict(text="📉 次日 + 後三日趨勢折線（黃點=預測中間價，帶狀=10%~90%區間）",
                   font=dict(size=15)),
...
def build_main_chart(df, fdates, paths, stop_price):
    ...
    fig = make_subplots(
        ...
        subplot_titles=("📈 股價 + 布林通道 + 預測區間", "MACD", "KD 指標", "成交量")
    )
...
def make_full_table(last_close, fdates, paths, stop_price):
    ...
    directions = []
    for p in up_prob:
        if p >= 58:   directions.append("⬆️ 偏漲")
        elif p <= 42: directions.append("⬇️ 偏跌")
...
with st.sidebar:
    ...
    noise_mult    = st.slider("波動倍數", 0.5, 3.0, 1.2, 0.1,
                               help="建議 1.2~1.5，確保區間夠寬")
...
if run_btn:
    ...
    t0    = sf[0]
    arrow = "🟢⬆️" if t0["up_prob"] >= 58 else ("🔴⬇️" if t0["up_prob"] <= 42 else "🟡↔️")

    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("現價",          f"{last_close:.2f}")
    c2.metric("次日預測價",    f"{t0['med']:.2f}", delta=f"{t0['chg_pct']:+.2f}%")
    c3.metric("上漲機率",      f"{t0['up_prob']:.1f}%",
              delta="偏漲✅" if t0["up_prob"] >= 58 else ("偏跌❌" if t0["up_prob"] <= 42 else "盤整⚠️"))
    c4.metric("低估區間(10%)", f"{t0['p10']:.2f}")
    c5.metric("高估區間(90%)", f"{t0['p90']:.2f}")

    if t0["up_prob"] >= 58:
        st.success(f"**{arrow} 次日偏漲**｜預測價 **{t0['med']:.2f}**，預期上漲 **{t0['chg_pct']:+.2f}%**，合理落點 {t0['p10']:.2f} ~ {t0['p90']:.2f}")
    elif t0["up_prob"] <= 42:
        st.error(f"**{arrow} 次日偏跌**｜預測價 **{t0['med']:.2f}**，預期下跌 **{t0['chg_pct']:+.2f}%**，合理落點 {t0['p10']:.2f} ~ {t0['p90']:.2f}")
    else:
        st.warning(f"**{arrow} 次日盤整**｜預測價 **{t0['med']:.2f}**，漲跌幅 **{t0['chg_pct']:+.2f}%**，合理落點 {t0['p10']:.2f} ~ {t0['p90']:.2f}")

    # ══════════════════════════════════════════
    # 區塊二：漲跌機率長條圖（清楚版，取代 Gauge）
    # ══════════════════════════════════════════
    st.markdown("---")
    st.plotly_chart(build_prob_bar_chart(sf), use_container_width=True)
    st.caption("🟢 ≥58% 偏漲　🟡 42~58% 盤整　🔴 ≤42% 偏跌｜長條越高代表上漲機率越大")
    ...
    st.caption("黃色折線 = 預測中間價；藍帶 = 10%~90% 合理區間；每格下方標示漲跌幅")
    ...
    for col, r in zip(cols, sf[1:]):
        emoji = "🟢" if r["up_prob"] >= 58 else ("🔴" if r["up_prob"] <= 42 else "🟡")
        col.markdown(f"""
...
| 方向判斷 | **{r['direction']}** |
    ...
    # 整體趨勢
    overall_up   = sum(1 for r in sf if r["up_prob"] >= 58)
    overall_down = sum(1 for r in sf if r["up_prob"] <= 42)
    total_chg_pct= (sf[-1]["med"] - last_close) / last_close * 100
    st.markdown("---")
    if overall_up >= 3:
        st.success(f"📈 **4日整體偏多**（{overall_up}/4日偏漲）｜累計預估漲幅 **{total_chg_pct:+.2f}%**（{last_close:.2f} → {sf[-1]['med']:.2f}）")
    elif overall_down >= 3:
        st.error(f"📉 **4日整體偏空**（{overall_down}/4日偏跌）｜累計預估跌幅 **{total_chg_pct:+.2f}%**（{last_close:.2f} → {sf[-1]['med']:.2f}）")
    else:
        st.warning(f"↔️ **4日整體震盪**｜累計預估變化 **{total_chg_pct:+.2f}%**（{last_close:.2f} → {sf[-1]['med']:.2f}）")
    ...
    if score >= 2:
        st.success(f"✅ 多指標偏多 → **最多可買 {shares:,} 股**，停損設 **{stop_price:.2f}**（-{(last_close-stop_price)/last_close*100:.1f}%）")
    elif score <= -2:
        st.error("❌ 多指標偏空 → **建議觀望，不要買**")
    else:
    ...
    def color_prob(val):
        try:
            v = float(val)
            if v >= 58: return "background-color:rgba(39,174,96,0.25)"
            if v <= 42: return "background-color:rgba(231,76,60,0.25)"
        except: pass
        return ""
